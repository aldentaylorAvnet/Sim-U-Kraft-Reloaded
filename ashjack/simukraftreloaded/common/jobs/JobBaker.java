...
import ashjack.simukraftreloaded.common.FarmingBox.FarmType;
...
public class JobBaker extends Job implements Serializable
{
    ...
    private void stageSellingBread()
    {
        ...
        else if (step == 3)
        {
            ...
            {
                sell = ModSimukraft.theFolks.size() + 1 + new Random().nextInt(ModSimukraft.theFolks.size());
                bakeryChests = inventoriesFindClosest(theFolk.employedAt, 4);
            ...
            if (breadStack == null)
            {
                ModSimukraft.sendChat(theFolk.name + " did not have any bread to sell today, do you have an active wheat farm?");
            }
            else
            {
                ModSimukraft.sendChat(theFolk.name + " has sold " + breadStack.stackSize
