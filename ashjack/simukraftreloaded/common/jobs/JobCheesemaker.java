...
public class JobCheesemaker extends Job {
    ...
    /*private void openCloseMilkShutters(boolean open) { TODO Get this working
    	...
    	if (shutters==null || shutters.size()<1) {
    		theFolk.selfFire();
        	ModSimukraft.sendChat("There was a problem with the Cheese factory, try re-building it - no shutters");
        	return;
    	}
    	Block newId=null;
    	if (open) {
	...
	private void stageEmptyingMilk() {
		if (step==1) {
			...
			int lightID = Block.getIdFromBlock(ModSimukraft.lightBox);
			ModSimukraft.log.info(Integer.toString(lightID));
			...
			for(V3 milkBlock:milkblocks) {
				...
				if (id==Blocks.air || (id ==ModSimukraft.blockFluidMilk && meta ==1)) {
					jobWorld.setBlock(milkBlock.x.intValue(), milkBlock.y.intValue(), milkBlock.z.intValue()
							, ModSimukraft.blockFluidMilk, 0, 0x03);
					try {theFolk.inventory.remove(0);}catch(Exception e){ModSimukraft.log.info("Caught Exception" + e.toString());}  //just in case!
					filledOk=true;
					break;
				}
			}
