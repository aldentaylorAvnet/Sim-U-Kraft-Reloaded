...
public class JobLivestockFarmer extends Job implements Serializable
{
    ...
    private void stageSlaughtering()
    {
        ...
        if (vocation == Vocation.CATTLEFARMER)
        {
            ...
            ok=inventoriesPut(farmChests, new ItemStack(Items.beef, quant), true);
            inventoriesPut(farmChests, new ItemStack(Items.leather, 1), false);
        }
        else if (vocation == Vocation.PIGFARMER)
        {
            quant = rand.nextInt(2) + 1;
            ok=inventoriesPut(farmChests, new ItemStack(Items.porkchop, quant), true);
        }
        else if (vocation == Vocation.CHICKENFARMER)
        {
            quant = rand.nextInt(2) + 1;
            ok=inventoriesPut(farmChests, new ItemStack(Items.chicken, quant), true);
            inventoriesPut(farmChests, new ItemStack(Items.feather, 1), false);
        }
