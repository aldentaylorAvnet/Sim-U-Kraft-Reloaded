	...
	*/
    ...
    private void doPickup(String buildingSearch,Item pickUpItem, boolean doCompareMeta) {
    	if (step==1) {
    		...
    		theFolk.statusText="Buying items at the "+pickupBuildings.get(currentPickup).displayName;
    		ArrayList<IInventory> chests=inventoriesFindClosest(pickupBuildings.get(currentPickup).primaryXYZ, 5);
    		ModSimukraft.log.info(chests.toString());
    		if (!chests.isEmpty()) {
    			ModSimukraft.log.info("Chest not empty");
    			
	    		int count=getItemCountInChests(chests, new ItemStack(pickUpItem, 1), doCompareMeta);
	    		
	    		ModSimukraft.log.info(Integer.toString(count));
	    		int buy=count/4;
	    		ModSimukraft.log.info(Integer.toString(buy));
	    		if (buy>0) {
		    		...
		    		if(pickUpItem == ModSimukraft.itemFood || pickUpItem == ModSimukraft.itemFoodCheese)
		    		{
		    			cheeseCount = count/4;
		    		}
    ...
    private void stagePickupCheese() {
    	if (step<4) {
    		doPickup("cheese factory",new ItemStack(ModSimukraft.itemFood,1,0).getItem(),true);
    	} else {
    ...
    private void stageDropoff() {
    	if (step==1) {
    		...
    		ModSimukraft.log.info("Unloading ingredients");
    		theFolk.statusText="Unloading ingredients";
    		ArrayList<V3> back=theStore.getSpecialBlocks(0);
    		ModSimukraft.log.info(back.get(0).toString());
    		ArrayList<IInventory> backstoreChests=inventoriesFindClosest(back.get(0), 3);
    		ModSimukraft.log.info(backstoreChests.get(0).toString());
    		...
    		for(int i=0; i<backstoreChests.get(0).getSizeInventory(); i++)
    		{
    			...
    			if(backstoreChests.get(0).getStackInSlot(i) != null)
    			{
    				...
    				if(chest.getItem() == ModSimukraft.itemFood && chest.stackSize < 64-cheeseCount)
    				{
    					backstoreChests.get(0).setInventorySlotContents(i, new ItemStack(ModSimukraft.itemFood, chest.stackSize + cheeseCount, 0));
    					placedCheese = true;
    					cheeseCount = 0;
    				}
    			...
    			if(backstoreChests.get(0).getStackInSlot(i) == null)
    			{
    				...
    				{
    					backstoreChests.get(0).setInventorySlotContents(i, new ItemStack(ModSimukraft.itemFood, cheeseCount, 0));
    					placedCheese = true;
    					cheeseCount = 0;
    					return;
    				}
